...
def generate_pdf_report(session_token, user_meta, top_5_results):
    ...
    info_text = (
        ...
        "당신의 상위 5대 핵심 잠재 역량입니다. 각 강점의 정의를 분석해보고, 일상 속에서 강화할 수 있는 "
        "행동 전략의 기준으로 삼아 보시기 바랍니다."
    )
    ...
    for idx, r in enumerate(top_5_results, 1):
        ...
        if keywords:
            pdf.set_font("NanumGothic", "B", 10)
            pdf.set_text_color(50, 50, 50)
            pdf.cell(0, 6, "🏷️ 연관 키워드", ln=True)
            
            pdf.set_font("NanumGothic", "", 9.5)
            pdf.set_text_color(100, 100, 100)
            keyword_str = ", ".join(keywords)
            pdf.cell(0, 6, f"  {keyword_str}", ln=True)
            pdf.ln(5)
